...
def get_first_derivative(x):
    ...
    i = len(sys.argv) - 2
    while (i >= 2):
        ret += (i - 2) * float(sys.argv[i]) * math.pow(x, i - 3)
        i -= 1
    return ret
...
def my_function(x):
    ...
    i = len(sys.argv) - 2
    while (i >= 2):
        ret += float(sys.argv[i]) * math.pow(x, i - 2)
        i -= 1
    return ret
...
def newton():
    ...
    while (round(my_function(x), int(sys.argv[len(sys.argv) - 1])) != 0):
        if (1 >= x >= 0 and out == 1):
            out = 0
        elif ((1 < x or x < 0) and out == 0):
            sys.exit(84)
        print("x = %.{}f".format(sys.argv[len(sys.argv) - 1]) % x)
        tmp_x = x
        x = tmp_x - my_function(tmp_x) / get_first_derivative(tmp_x)
...
def bisection():
    ...
    i = 0
    start_time = time.time()
    while (abs(higher - lower) > math.pow(10, -int(sys.argv[len(sys.argv) - 1]))):
        ...
        if (i < int(sys.argv[7])):
            print("x = %.{}g".format(sys.argv[len(sys.argv) - 1]) % middle)
        else:
            print("x = %.{}f".format(sys.argv[len(sys.argv) - 1]) % middle)
        i += 1
        if (float(time.time()) - float(start_time) > 9.0):
            os.system('clear')
            sys.exit(84)
...
def secant():
    ...
    first = True
    start_time = time.time()
    while(round(my_function(x), int(sys.argv[len(sys.argv) - 1]) - 1) != 0):
        ...
        if (float(time.time()) - float(start_time) > 9.0):
            ...
            sys.exit(84)
        if (first):
            print("x = %.{}g".format(sys.argv[len(sys.argv) - 1]) % x)
        else:
            print("x = %.{}f".format(sys.argv[len(sys.argv) - 1]) % x)
        first = False
    sys.exit(0)
...
def my_input():
    if (len(sys.argv) == 2 and sys.argv[1] == "-h"):
        ...
        print("    a[0-infinity and beyond]    coefficients of the equation")
